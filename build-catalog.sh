...

{
    ...
    printf 'author\tlicence\tsize\tpicture\tpicture-digest\tdemo\tfeatured\n'
} > "$tmp"
...
while IFS=$'\t' read -r name kind version prefixes source target requires options summary \
    ...
    author licence size picture demo featured || [ -n "${name:-}" ]; do
    [ -n "${name:-}" ] || continue
    case "$name" in \#*) continue ;; esac
    if [ -z "${featured:-}" ]; then
        echo "$name: expected 24 tab-separated columns" >&2
        failed=1
        continue
    fi
    case "$name" in
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$author" "$licence" "$size" "$picture" "$picture_digest" "$demo" "$featured" >> "$tmp"
    count=$((count + 1))
done < "$items"
