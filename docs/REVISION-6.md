...
| Row | Content |
|---|---|
| 0 | Masthead. Front: the 3×5 pixel TLSTORE mark in square pixels over rows 0–1, blocky like dawn's name (half-block letters where pictures cannot be shown; tap: home; plain bold `tlstore` in one row where the header has no blank row or no room), and, right-aligned, `↑ N updates` in the accent when N > 0 (tap: the updates filter) with `N items · M installed` on row 1 under it; with no updates the counts sit on row 0, level with the mark's top. The counts are a caption: two-thirds size (OSC 66 `n=2:d=3`, as the category tags) in `rule`. Item and Installing: `‹ apps` at the gutter (`‹` accent, `apps` dim; tap: back) and the upstream `owner/repo ↗` right-aligned, accent, curly underline, OSC 8 link; a setup shows `our setup` dim instead. |
...
| 3 … 2+P | Picture, P rows, a card the full 49-column width: rounded corners (8 % of its shorter side, 4–24 px) and a 1.5 px hairline border in `rule`, no fade. Fitted to the width; a picture taller than P rows keeps the band with the most detail, weighted to its middle (chosen on a clip's first frame, then fixed for every frame). P = spare rows after the fixed rows, clamped to at most 12 and to the picture's aspect height; under 4 the picture is not drawn and its rows go to the body. The catalog picture on Front; the README's first image on Item and Installing (falling back to the catalog picture). Fetched through `tlstore picture` / `tlstore readme-asset` off the draw path; nothing is drawn where a picture has not arrived. |
