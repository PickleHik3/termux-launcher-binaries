...
ctx.pics: .script_word(text,px_h,Rgb) /*Pinyon*/ .mark(px,Rgb) .file(path,w,h,Fit) .header_picture(path,w,h,animate) /*Fit::Width (fills the width, keeps the busiest band) framed as a card (rounded, `card_edge` hairline);
