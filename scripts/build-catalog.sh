...
# payload must never reach a phone. An item's picture, pinned readme and demo
# are each hashed the same way (always a launcher:/binaries: source, or "-"
# when there is none) and their digests ride alongside them as their own
...
# names a path in the binaries repository directly, looked up in SHA256SUMS by
# that exact repo-relative path; a bare asset (e.g. binaries:dawn@tag) keeps
